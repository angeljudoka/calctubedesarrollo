...
/**
 * Description of CableController
 *
...
use BackendBundle\Entity\Cable;
    ...
    public function newAction(Request $request) {
        ...
        if ($authCheck == true) {
            ...
            if ($json != null) {
                ...
                $diametroExterior = (isset($params->diametro)) ? $params->diametro : null;
                $cable_tipo = (isset($params->cableTipo)) ? $params->cableTipo : null;
                ...
                if ($usuario_id != null) {
                    ...
                    $nombreBase = $em->getRepository("BackendBundle:Cable")->findOneBy(
                            array(
                                "nombre" => $nombre
                    ));
                    
                    $cableTipo = $em->getRepository("BackendBundle:CableTipo")->findOneBy(
                            array(
                                "id" => $cable_tipo
                    ));
                    ...
                    $cable->setDiametroExterior($diametroExterior);
                    $cable->setCableTipo($cableTipo);

                    if ($nombre == null) {
                        $data = array(
                            "status" => "error",
                            "code" => 400,
                            "msg" => "Para poder dar de alta a un cable, se deben llenar los campos marcados en rojo, intente de nuevo !"
                        );
                    } else {
                        if (isset($usuario) && $nombreBase == null) {
                            ...
                            $data = array(
                                "status" => "error",
                                "code" => 400,
                                "msg" => "Es posible que El cable que se quiere dar de alta ya este registrada en el sistema, intente de nuevo"
                            );
                        }
                    }
                } else {
        ...
        if ($authCheck == true) {
            ...
            if ($json != null) {
                ...
                if ($usuario_id != null && $cable_id != null) {
                    ...
                    $cable = $em->getRepository("BackendBundle:Cable")->findOneBy(
                            array(
                                "id" => $cable_id
                    ));
        ...
        if ($authCheck == true) {
            ...
            if (is_object($cable) && $usuario_id != null) {
                ...
                $data = array(
                    "status" => "success",
                    "code" => 200,
                    "msg" => "Se ha eliminado el cable con exito !"
                );
            } else {
                $data = array(
                    "status" => "error",
                    "code" => 400,
                    "msg" => "El cable especificado no existe, intente de nuevo"
                );
            }
        } else {
            $data = array(
    ...
    public function detailAction(Request $request, $id = null) {
        ...
        if ($cable) {
            ...
            $data = array(
                "status" => "error",
                "code" => 400,
                "msg" => "El cable no existe, intente con uno nuevo"
            );
        }
